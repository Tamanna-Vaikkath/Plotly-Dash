...
def build_variance_story_tab():
    ...
    return html.Div([
        ...
        dbc.Alert([
            ...
            html.Strong("Feature Deep Dive — See Exactly Where the GLM Fails: "),
            "Select any variable to see how the GLM (red dashed, linear) compares to "
            "the GA2M (green curve, non-linear) on actual loss data. "
            "The binned mean loss curve shows the organized relationship — "
            "and exactly where the GLM's linear assumption breaks down.",
        ], color="warning", className="mb-3",
           style={"borderLeft": f"4px solid {GOLD}", "backgroundColor": "#FFFBF0",
                  "borderRadius": "8px", "fontSize": "0.85rem"}),
        ...
        dbc.Row([
            dbc.Col([
                dbc.Card([dbc.CardBody([
                    html.Div("SELECT FEATURE TO ANALYSE",
                    ...
                    dcc.Dropdown(
                        id="feature-dive-dd",
                        options=[
                            {"label": "Roof Age",          "value": "Roof_Age_Applicant"},
                            {"label": "Roof Vulnerability", "value": "Roof_Vulnerability_Satellite"},
                            {"label": "Dwelling Age",       "value": "Dwelling_Age"},
                            {"label": "Hail Frequency",     "value": "Hail_Frequency"},
                        ],
                    ...
                    html.Div(id="feature-dive-meta",
                             style={"marginTop": "14px", "fontSize": "0.78rem",
            ...
            dbc.Col([
                chart_card(
                    ...
                    "tt-dive-overlay",
                    "Grey bars = mean actual loss per bin (organised, not scattered). "
                    "Red dashed line = GLM linear assumption. "
                    "Green solid curve = GA2M learned non-linear shape. "
                    "The gap between red and green is the structural premium leakage.",
                    dcc.Loading(
                        dcc.Graph(id="feature-dive-overlay",
                                  config={"displayModeBar": False}),
                        type="circle"),
                    subtitle="Bars = mean actual loss per bin · Red dashed = GLM · Green = GA2M non-linear",
                ),
            ], width=9),
        ], className="g-3 pb-4"),
...
# Drives: feature-dive-overlay, feature-dive-meta
# Supports 4 fixed features: Roof_Age_Applicant, Roof_Vulnerability_Satellite,
#   Dwelling_Age (derived), Hail_Frequency
# ══════════════════════════════════════════════════════════════════════════════
@callback(
...
def update_feature_dive(feature):
    _GLM_LINE = "#E63946"
    _GAM_LINE = "#2D8A50"
    _BAR_COL  = "rgba(160,185,220,0.55)"
    _BAR_LINE = "rgba(100,140,190,0.6)"
    ...
    LAY_BASE = dict(
        ...
        plot_bgcolor="#FAFBFC",
        paper_bgcolor=WHITE,
        margin=dict(l=60, r=40, t=20, b=55),
    )

    # ── Feature config: label, EBM key, x-axis title ─────────────────────────
    # Dwelling_Age is a derived feature — present in df but may need explicit
    # source column handling. Map to the correct EBM feature name.
    _FEAT_MAP = {
        "Roof_Age_Applicant":        "Roof_Age_Applicant",
        "Roof_Vulnerability_Satellite": "Roof_Vulnerability_Satellite",
        "Dwelling_Age":              "Dwelling_Age",   # derived EBM feature
        "Hail_Frequency":            "Hail_Frequency",
    }
    _FEAT_LABELS = {
        "Roof_Age_Applicant":           "Roof Age (years)",
        "Roof_Vulnerability_Satellite": "Roof Vulnerability Score",
        "Dwelling_Age":                 "Dwelling Age (years)",
        "Hail_Frequency":               "Hail Frequency (events/year)",
    }
    _FEAT_STORIES = {
        "Roof_Age_Applicant": {
            "what": "How old the roof is in years — one of the strongest non-linear risk signals.",
            "glm": "The GLM treats every additional roof year identically — a flat slope from age 0 to 40. It cannot detect the acceleration in damage probability that occurs after year 20.",
            "gam": "GA2M learns that risk is nearly flat for the first 15 years, then curves sharply upward. A 25-year-old roof can carry 3–4× more risk than the GLM prices — especially combined with hail or wildfire exposure.",
            "so_what": "Old roofs in hail-prone or wildfire zones are chronically underpriced by the GLM. GA2M applies a targeted surcharge that grows non-linearly with roof age, recovering the premium leakage.",
        },
        "Roof_Vulnerability_Satellite": {
            "what": "Satellite-assessed roof condition score — a modern enrichment signal not available in traditional GLMs.",
            "glm": "The GLM cannot use satellite imagery data at all. Even if it could, it would apply a fixed linear relativity — missing the threshold where roof deterioration suddenly accelerates.",
            "gam": "GA2M learns a quadratic relationship: low scores carry manageable risk, but past a vulnerability threshold the damage curve bends steeply upward. This is the compounding effect of material degradation.",
            "so_what": "High-vulnerability roofs that look acceptable on traditional metrics are significantly underpriced. GA2M's shape function identifies the deterioration cliff and applies the correct surcharge.",
        },
        "Dwelling_Age": {
            "what": "How old the home structure is — captures aging electrical, plumbing, and foundation risk that accumulates non-linearly over time.",
            "glm": "The GLM applies a constant aging factor — the same risk increment per year whether a home is 10 years old or 60 years old. It cannot model the compounding failure points that emerge in very old homes.",
            "gam": "GA2M learns that aging risk is relatively contained for newer homes but accelerates past 40–50 years as multiple systems fail simultaneously. The curve shows a distinct threshold effect the GLM's straight line misses entirely.",
            "so_what": "Homes past 40 years are systematically underpriced. GA2M applies an age-accelerating surcharge that reflects the true compounding risk of an aging housing stock.",
        },
        "Hail_Frequency": {
            "what": "Number of significant hail events per year at this property location — a key geographic peril signal.",
            "glm": "The GLM adds the same fixed premium increment per additional hail event per year. It treats a location with 6 events/year as simply 6× the single-event risk — missing the threshold effect.",
            "gam": "GA2M learns that hail risk has a threshold: below 2 events/year the exposure is manageable, but above 4 events/year, cumulative roof stress causes exponential damage acceleration. The jump is sharp and discontinuous — exactly the kind of pattern a straight line cannot capture.",
            "so_what": "Properties in the TX/OK/CO hail belt with frequent events are chronically underpriced. GA2M's threshold shape function applies a non-linear surcharge that rises steeply past the frequency inflection point.",
        },
    }
    ...
    def _empty_fig(msg):
        f = go.Figure().add_annotation(
            ...
            showarrow=False, font=dict(size=13, color=MUTED, family="Inter"))
        f.update_layout(**LAY_BASE, height=440)
        return f

    if feature not in _FEAT_MAP:
        return _empty_fig("Select a risk factor from the dropdown"), []

    _ebm_feat  = _FEAT_MAP[feature]
    feat_label = _FEAT_LABELS.get(feature, feature.replace("_", " "))
    story      = _FEAT_STORIES.get(feature, {})

    # ── Pull values — use df filtered to test split for robustness ────────────
    # This ensures Dwelling_Age (derived at load time on df) is always available
    _src = df[df["Split"] == "test"].copy() if "Split" in df.columns else _test.copy()

    # Ensure Dwelling_Age is present
    if "Dwelling_Age" not in _src.columns:
        _src["Dwelling_Age"] = (2026 - _src["Year_Built"]).astype(int)

    if feature not in _src.columns:
        return _empty_fig(f"Feature '{feat_label}' not found in data"), []

    try:
        _fv = pd.to_numeric(_src[feature], errors="coerce")
    except Exception:
        return _empty_fig("Could not read feature values"), []

    _valid  = _fv.notna()
    _fx     = _fv[_valid].values
    _actual = _src.loc[_valid, "Expected_Pure_Premium"].values
    _glm    = _src.loc[_valid, "GLM_Pure_Premium"].values
    _gam    = _src.loc[_valid, "Final_Pure_Premium"].values

    if len(_fx) < 20:
        return _empty_fig("Not enough data for this feature"), []

    # ── Bin into quantile buckets ─────────────────────────────────────────────
    N_BINS = 25
    try:
        _bins = pd.qcut(_fx, q=N_BINS, duplicates="drop")
    except Exception:
        try:
            _bins = pd.cut(_fx, bins=N_BINS)
        except Exception:
            return _empty_fig("Could not bin feature values"), []

    _bc, _ma, _mg, _mgam, _mse = [], [], [], [], []
    _bins_arr = np.asarray(_bins)   # works whether _bins is Series or ndarray
    for _b in sorted(_bins.dropna().unique(), key=lambda x: x.mid):
        _m = _bins_arr == _b
        if _m.sum() < 3:
            continue
        _bc.append(float(_b.mid))
        _ma.append(float(_actual[_m].mean()))
        _mg.append(float(_glm[_m].mean()))
        _mgam.append(float(_gam[_m].mean()))
        _mse.append(float(_actual[_m].std() / max(np.sqrt(_m.sum()), 1)))

    if len(_bc) < 3:
        return _empty_fig("Not enough distinct bins — try another feature"), []

    _bc   = np.array(_bc, dtype=float)
    _ma   = np.array(_ma, dtype=float)
    _mg   = np.array(_mg, dtype=float)
    _mgam = np.array(_mgam, dtype=float)
    _mse  = np.array(_mse, dtype=float)

    # ── GLM: fit a straight line through the binned GLM means ─────────────────
    _glm_poly = np.polyfit(_bc, _mg, 1)
    _glm_line = np.polyval(_glm_poly, _bc)

    # ── GA2M curve: native EBM shape if available, else smoothed bin means ────
    _gam_curve = pd.Series(_mgam).rolling(3, center=True, min_periods=1).mean().values
    try:
        if _ebm_feat in EBM_ALL_FEATURES:
            _fidx  = EBM_ALL_FEATURES.index(_ebm_feat)
            _fdata = global_exp.data(_fidx)
            if (_fdata and "names" in _fdata and "scores" in _fdata
                    and len(_fdata["names"]) > 1):
                _sfx = np.array(_fdata["names"], dtype=float)
                _sfy = np.array(_fdata["scores"], dtype=float)
                # Guard: must be 1D arrays of matching shape
                if _sfx.ndim == 1 and _sfy.ndim == 1 and len(_sfx) == len(_sfy):
                    _srt = np.argsort(_sfx)
                    _sfx_s, _sfy_s = _sfx[_srt], _sfy[_srt]
                    _interp = np.interp(_bc, _sfx_s, _sfy_s)
                    # Convert log-uplift → dollar premium scale
                    _base = float(np.median(_mg))  # median GLM as base
                    _raw  = _base * np.exp(_interp)
                    # Scale so the curve passes through the binned GA2M mean at the median
                    _med_idx = len(_bc) // 2
                    _scale   = float(_mgam[_med_idx]) / max(float(_raw[_med_idx]), 1)
                    _raw_scaled = _raw * _scale
                    _gam_curve  = pd.Series(_raw_scaled).rolling(3, center=True,
                                                                   min_periods=1).mean().values
    except Exception:
        pass  # fall back to smoothed bin means

    # ── Y-axis: robust cap ────────────────────────────────────────────────────
    _y_cap    = float(np.percentile(_ma, 97))
    _ma_plot  = np.clip(_ma,       0, _y_cap * 1.12)
    _glm_plot = np.clip(_glm_line, 0, _y_cap * 1.20)
    _gam_plot = np.clip(_gam_curve,0, _y_cap * 1.20)

    # ── Build figure ──────────────────────────────────────────────────────────
    fig = go.Figure()

    # 1. Actual loss bars
    _bw = float(np.median(np.diff(_bc))) * 0.82 if len(_bc) > 1 else 1.0
    fig.add_trace(go.Bar(
        x=_bc, y=_ma_plot, width=_bw,
        name="Actual Loss (avg per group)",
        marker_color=_BAR_COL,
        marker_line_color=_BAR_LINE, marker_line_width=0.8,
        error_y=dict(type="data", array=np.clip(_mse, 0, _y_cap * 0.12),
                     visible=True, thickness=1.2, width=3,
                     color="rgba(100,140,190,0.45)"),
        hovertemplate=(
            f"<b>{feat_label} ≈ %{{x:.1f}}</b><br>"
            "Average actual loss: <b>$%{y:,.0f}</b><br>"
            "<i>Mean of all policies in this group</i>"
            "<extra></extra>"
        ),
    ))
    ...
    fig.add_trace(go.Scatter(
        x=_bc, y=_glm_plot, mode="lines",
        name="GLM — straight-line assumption",
        line=dict(color=_GLM_LINE, width=2.5, dash="dash"),
        hovertemplate=(
            f"{feat_label}: %{{x:.1f}}<br>"
            "GLM charges: <b>$%{y:,.0f}</b><br>"
            "<i>GLM assumes risk grows in a constant straight line</i>"
            "<extra></extra>"
        ),
    ))

    # 3. GA2M non-linear curve
    fig.add_trace(go.Scatter(
        x=_bc, y=_gam_plot, mode="lines",
        name="GA2M — true risk curve",
        line=dict(color=_GAM_LINE, width=3),
        hovertemplate=(
            f"{feat_label}: %{{x:.1f}}<br>"
            "GA2M charges: <b>$%{y:,.0f}</b><br>"
            "<i>GA2M learned the actual non-linear shape from data</i>"
            "<extra></extra>"
        ),
    ))

    # 4. Shaded mispricing gap between GLM and GA2M
    _above = _gam_plot > _glm_plot  # GA2M above GLM = GLM was underpricing
    fig.add_trace(go.Scatter(
        x=list(_bc) + list(_bc[::-1]),
        y=list(np.where(_above, _gam_plot, _glm_plot)) +
          list(np.where(_above, _glm_plot, _gam_plot)[::-1]),
        fill="toself",
        fillcolor="rgba(230,57,70,0.07)",
        line=dict(width=0), showlegend=False, hoverinfo="skip",
    ))

    # 5. Peak divergence annotation in plain client language
    _div  = np.abs(_gam_plot - _glm_plot)
    _pidx = int(np.argmax(_div))
    _px   = float(_bc[_pidx])
    _pgap = float(_gam_plot[_pidx] - _glm_plot[_pidx])
    _is_under = _pgap > 0   # positive = GLM was underpricing

    _ann_ax = -80 if _pidx > len(_bc) * 0.55 else 80
    _ann_ay = -50

    fig.add_annotation(
        x=_px, y=float(_gam_plot[_pidx]),
        text=(
            f"<b>{'Underpriced' if _is_under else 'Overpriced'} by GLM</b><br>"
            f"At {feat_label.split(' (')[0]} = {_px:.1f}:<br>"
            f"GLM charges ${float(_glm_plot[_pidx]):,.0f}<br>"
            f"True risk = ${float(_gam_plot[_pidx]):,.0f}<br>"
            f"<b>Gap: {'+'if _is_under else '-'}${abs(_pgap):,.0f} / policy</b>"
        ),
        showarrow=True, arrowhead=2, arrowcolor=NAVY,
        ax=_ann_ax, ay=_ann_ay,
        font=dict(size=9, color=NAVY, family="Inter"),
        bgcolor=WHITE,
        bordercolor=RED if _is_under else GREEN,
        borderwidth=2, borderpad=6, align="left",
    )
    fig.add_vline(x=_px, line_color="#CCCCCC", line_width=1.3, line_dash="dot")

    fig.update_xaxes(
        title_text=feat_label, gridcolor="#F0F2F5",
        tickfont=dict(size=10, family="Inter"),
        title_font=dict(size=11, family="Inter"),
    )
    fig.update_yaxes(
        title_text="Average Premium ($)", tickprefix="$",
        gridcolor="#F0F2F5",
        tickfont=dict(size=10, family="Inter"),
        title_font=dict(size=11, family="Inter"),
        range=[0, _y_cap * 1.25],
    ...
    # ── Meta panel: plain-English story, no R² ───────────────────────────────
    _lift_arr = np.abs(_glm_line - _ma) - np.abs(_gam_curve - _ma)
    _avg_lift = float(np.mean(_lift_arr))
    _pct_win  = float((_lift_arr > 0).mean())
    ...
    meta = [
        ...
        html.Div([
            ...
            html.Strong(feat_label.split(" (")[0],
                        style={"color": NAVY, "fontSize": "0.9rem"}),
        ], className="d-flex align-items-center mb-3"),

        # What this measures
        html.Div([
            html.Div("WHAT THIS MEASURES",
                     style={"fontSize": "0.65rem", "color": MUTED,
                            "fontWeight": "700", "letterSpacing": "0.07em",
                            "marginBottom": "3px"}),
            html.Div(story.get("what", feat_label),
                     style={"fontSize": "0.80rem", "color": NAVY,
        ...
        html.Div([
            html.Div([
                ...
                html.Span("WHAT THE GLM ASSUMES",
                          style={"fontSize": "0.65rem", "color": MUTED,
                                 "fontWeight": "700", "letterSpacing": "0.07em"}),
            ], className="mb-1"),
            html.Div(story.get("glm", "The GLM applies a constant linear slope."),
                     style={"fontSize": "0.79rem", "color": "#B03020",
                            "lineHeight": "1.5", "marginBottom": "12px",
                            "paddingLeft": "12px",
        ...
        html.Div([
            html.Div([
                ...
                html.Span("WHAT GA2M LEARNS",
                          style={"fontSize": "0.65rem", "color": MUTED,
                                 "fontWeight": "700", "letterSpacing": "0.07em"}),
            ], className="mb-1"),
            html.Div(story.get("gam", "GA2M learns the true non-linear shape."),
                     style={"fontSize": "0.79rem", "color": "#145A32",
                            "lineHeight": "1.5", "marginBottom": "12px",
                            "paddingLeft": "12px",
        ...
        dbc.Row([
            dbc.Col([
                html.Div(f"${abs(_pgap):,.0f}",
                         style={"fontWeight": "800", "fontSize": "1.25rem",
                                "color": RED if _pgap > 0 else GREEN,
                                "lineHeight": "1"}),
                html.Div("peak mispricing",
                         style={"fontSize": "0.68rem", "color": MUTED,
                                "marginTop": "2px"}),
            ], width=6),
            dbc.Col([
                html.Div(f"${abs(_avg_lift):,.0f}",
                         style={"fontWeight": "800", "fontSize": "1.25rem",
                                "color": BLUE, "lineHeight": "1"}),
                html.Div("avg correction/policy",
                         style={"fontSize": "0.68rem", "color": MUTED,
                                "marginTop": "2px"}),
            ], width=6),
        ], className="g-0 mb-3"),
        ...
        # Business impact
        html.Div([
            html.Div("BUSINESS IMPACT",
                     style={"fontSize": "0.65rem", "color": MUTED,
                            "fontWeight": "700", "letterSpacing": "0.07em",
                            "marginBottom": "4px"}),
            html.Div(story.get("so_what",
                               f"GA2M corrects pricing on {_pct_win:.0%} of risk segments "
                               f"for this variable, recovering ${abs(_avg_lift):,.0f} per policy on average."),
                     style={"fontSize": "0.79rem", "color": MUTED,
